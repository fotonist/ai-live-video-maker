...
def _decode_audio(path: Path) -> np.ndarray:
    ...
    The returned array is intentionally the only full-audio buffer kept in memory.
    Analysis below avoids creating a second large 2-D frame matrix or a full
    spectrogram, which previously caused excessive memory consumption on Render.
    """
...
def _energy_curve(samples: np.ndarray) -> list[float]:
    ...
    values = [
        float(np.sqrt(np.mean(np.square(samples[start : start + window_size]))))
        for start in range(0, samples.size, window_size)
        if samples[start : start + window_size].size
    ]
    if not values:
        return [0.0]
    maximum = max(values)
...
def _onset_envelope(samples: np.ndarray) -> np.ndarray:
    """Calculate spectral-flux onsets without materializing a spectrogram.

    The previous implementation created a sliding-window matrix and then an
    entire FFT magnitude matrix. For a multi-minute song that can consume
    hundreds of MB. Processing one frame at a time keeps peak analysis memory
    essentially constant relative to song duration.
    """
    if samples.size < ANALYSIS_FRAME_SIZE:
        padded = np.pad(samples, (0, ANALYSIS_FRAME_SIZE - samples.size)).astype(np.float32, copy=False)
        frame_count = 1
    else:
        frame_count = 1 + (samples.size - ANALYSIS_FRAME_SIZE) // ANALYSIS_HOP_SIZE

    window = np.hanning(ANALYSIS_FRAME_SIZE).astype(np.float32)
    flux_values = np.empty(frame_count, dtype=np.float32)
    previous_spectrum: np.ndarray | None = None

    for index in range(frame_count):
        start = index * ANALYSIS_HOP_SIZE
        frame = samples[start : start + ANALYSIS_FRAME_SIZE]
        if frame.size < ANALYSIS_FRAME_SIZE:
            frame = np.pad(frame, (0, ANALYSIS_FRAME_SIZE - frame.size))
        spectrum = np.abs(np.fft.rfft(frame * window))
        if previous_spectrum is None:
            flux_values[index] = 0.0
        else:
            flux_values[index] = float(np.maximum(spectrum - previous_spectrum, 0.0).sum())
        previous_spectrum = spectrum

    if flux_values.size < 3:
        return flux_values

    baseline = np.convolve(flux_values, np.ones(9, dtype=np.float32) / 9.0, mode="same")
    novelty = np.maximum(flux_values - baseline, 0.0)
...
def _estimate_bpm(onsets: np.ndarray) -> float:
    ...
    return round(
        float(np.clip(60.0 * ANALYSIS_SAMPLE_RATE / (best_lag * ANALYSIS_HOP_SIZE), 50.0, 180.0)),
        2,
    )
...
def _beat_positions(bpm: float, duration: float) -> list[float]:
    return [
        round(float(value), 3)
        for value in np.arange(0.0, duration, 60.0 / bpm)
    ] if duration > 0 else []
...
def _sections(energy: list[float], duration: float) -> list[dict[str, float | str]]:
    ...
    labels = [
        "intro" if i == 0 else "chorus" if value >= high else "bridge" if value <= low else "verse"
    ...
    for index in range(1, len(labels)):
        if labels[index] != current:
            sections.append(
                {
                    "label": current,
                    "start": round(start_index * 0.5, 3),
                    "end": round(min(index * 0.5, duration), 3),
                }
            )
...
def _analyze_audio(project: Project) -> AudioAnalysisResponse:
    ...
    onsets = _onset_envelope(samples)
...
def analyze_audio(project_id: UUID, db: Session = Depends(get_db)) -> AudioAnalysisResponse:
    ...
    project.analysis.duration_seconds = result.duration_seconds
    project.analysis.sample_rate = result.sample_rate
    project.analysis.bpm = result.bpm
    project.analysis.beat_interval_seconds = result.beat_interval_seconds
    project.analysis.beat_positions_seconds = result.beat_positions_seconds
    project.analysis.energy_curve = result.energy_curve
    project.analysis.sections = result.sections
