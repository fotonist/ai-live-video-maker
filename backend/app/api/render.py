...
def render_project(project_id: UUID, output_format: str = "9:16", db: Session = Depends(get_db)) -> RenderResponse:
    ...
    result = run([
        ffmpeg, "-y", "-v", "error",
        "-f", "lavfi", "-i", f"color=c=0x0b1020:s={width}x{height}:r=30",
    ...
    if result.returncode != 0 or not video_path.exists():
        ...
        raise HTTPException(status_code=500, detail=f"Video render failed: {detail[:500]}")
