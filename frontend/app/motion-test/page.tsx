...
export default function MotionTestPage() {
  ...
  function selectAudio(event: ChangeEvent<HTMLInputElement>) {
    ...
    if (file && !file.type.startsWith("audio/")) {
      setError("Use an MP3 or WAV file.");
      setAudio(null);
      return;
    }
    setError("");
  ...
  async function start() {
    if (!image || !audio) {
      setError("Select one image and one audio file.");
      return;
    }
    ...
    try {
      ...
      setBusy(false);
      setError(err instanceof Error ? err.message : "Motion test failed.");
    }
  }
  ...
  async function poll(id: string) {
    try {
      ...
      timer.current = setTimeout(() => void poll(id), 3000);
    }
  }
    ...
    <main style={styles.page}>
      <section style={styles.shell}>
        ...
        <p style={styles.subtitle}>
          This is a 10-second proof of concept. No OpenAI key. No video-generation API. FFmpeg creates the camera movement locally on the Render service.
        </p>

        <div style={styles.grid}>
          <label style={styles.card}>
            <span style={styles.label}>1. Still image</span>
            <span style={styles.drop}>{image ? image.name : "Choose JPG, PNG or WEBP"}</span>
            <span style={styles.hint}>The image will receive slow zoom + pan motion.</span>
            <input type="file" accept="image/jpeg,image/png,image/webp" onChange={selectImage} style={styles.input} />
          </label>

          <label style={styles.card}>
            <span style={styles.label}>2. Music</span>
            <span style={styles.drop}>{audio ? audio.name : "Choose MP3 or WAV"}</span>
            <span style={styles.hint}>The first 10 seconds are used for this test.</span>
            <input type="file" accept="audio/mpeg,audio/wav,audio/x-wav,audio/*" onChange={selectAudio} style={styles.input} />
          </label>
        </div>

        <button type="button" onClick={start} disabled={busy} style={styles.button}>
          {busy ? "Rendering motion test..." : "Create 10-second motion test →"}
        </button>
        ...
        {job && (
          <div style={styles.result}>
            <div style={styles.statusRow}>
              <strong>{job.status === "completed" ? "Motion test complete" : job.status === "failed" ? "Motion test failed" : "Rendering..."}</strong>
              <span>{job.message}</span>
            </div>
            {job.status === "rendering" || job.status === "queued" ? <div style={styles.loader}><div style={styles.loaderBar} /></div> : null}
            {job.status === "failed" && <pre style={styles.error}>{job.error}</pre>}
            {video && (
              <div style={styles.videoWrap}>
                <video src={video} controls playsInline style={styles.video} />
                <a href={video} target="_blank" rel="noreferrer" style={styles.link}>Open MP4</a>
              </div>
            )}
          </div>
        )}
...
const styles: Record<string, React.CSSProperties> = {
  ...
  loaderBar: { height: "100%", width: "45%", background: "#7dd3fc", animation: "none" },
